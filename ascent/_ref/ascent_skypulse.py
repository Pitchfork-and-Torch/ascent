...
# DTN stacking (skill ascent-dtn-space-stacking). ASCENT is never a CLA.
STACK_STARLINK_IP = "D"
STACK_DEEP_SPACE = "A+C"
STACK_CLA_FORBIDDEN = "B"


def recommend_stack(path: str) -> dict[str, Any]:
    """Place ASCENT relative to DTN. Never recommend an ASCENT convergence layer.

    Starlink IP today: (D) ASCENT + optional ASCENT-D over UDP/QUIC/TCP, no BP.
    True deep space: (A)+(C) ASCENT ADU inside BPv7; ASCENT-D stays app integrity.
    (B) ASCENT-as-CLA is rejected: hop transport stays TCPCL/UDPCL/LTPCL.

    This does not raise Starlink RF Mbps. next_capacity remains a sender bottleneck hint.
    """
    p = (path or "").strip().upper().replace("_", "-")
    if p in (
        "B",
        "CLA",
        "CONVERGENCE",
        ...
        return {
            "placement": STACK_CLA_FORBIDDEN,
            "allowed": False,
            "bp": False,
            "ascent_d": False,
            "note": (
                "Reject (B): ASCENT is a content/agent wire, not a hop transport. "
                "Do not encode agent planes as LTP segments or invent an ASCENT CLA."
            ),
        }
    if p in (
        "DEEP",
        ...
        return {
            "placement": STACK_DEEP_SPACE,
            "allowed": True,
            "bp": True,
            "ascent_d": True,
            "note": (
                "Deep space: ADU = ASCENT frame(s) inside BPv7 (A). "
                "ASCENT-D RS(255,223) erase-on-fail is end-to-end app integrity (C). "
                "Hop reliability stays LTP/ECLSA/TM coding. Not a Starlink IP default."
            ),
        }
    return {
        "placement": STACK_STARLINK_IP,
        "allowed": True,
        "bp": False,
        "ascent_d": False,
        "note": (
            "Starlink IP / LEO with end-to-end IP: ASCENT over UDP/QUIC/TCP without BP/LTP. "
            "Optional P9 only for spool or high-BER, never as a PHY Mbps claim."
        ),
    }
