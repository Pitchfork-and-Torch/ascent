...
def test_spec_header() -> None:
    ...
    assert f"**Nexus / architecture card:** {NEXUS} Delay Hull" in spec
...
def test_readme_and_install() -> None:
    ...
    assert f"**Nexus:** {NEXUS} Delay Hull" in inst
...
def test_llms_and_site() -> None:
    ...
    assert f'"softwareVersion": "{NEXUS}"' in html
...
def test_docs_current_card() -> None:
    ...
    assert f"Nexus architecture card **{NEXUS}** Delay Hull" in sky
    bridge = _read("docs/ORBITSTACK-LEOAWARE-BRIDGE.md")
    assert f"wire **{WIRE}**" in bridge
    assert f"Nexus architecture card **{NEXUS}** Delay Hull" in bridge
