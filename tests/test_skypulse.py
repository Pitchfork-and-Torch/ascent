...
def test_dtn_stack_never_cla():
    """Starlink IP uses (D); deep space uses (A)+(C); ASCENT-as-CLA is rejected."""
    star = recommend_stack("starlink")
    assert star["placement"] == "D"
    assert star["allowed"] is True
    assert star["bp"] is False
    assert "RF" not in star["note"]
    deep = recommend_stack("bpv7")
    assert deep["placement"] == "A+C"
    assert deep["bp"] is True
    assert deep["ascent_d"] is True
    banned = recommend_stack("CLA")
    assert banned["allowed"] is False
    assert banned["placement"] == "B"
    assert "not a hop transport" in banned["note"].lower() or "CLA" in banned["note"]
    print("PASS test_dtn_stack_never_cla")
