...
**Version:** 1.0.0-rc1  
**Nexus / architecture card:** 4.0.0 Delay Hull  
**Wire packages (PyPI/npm ascent-wire):** 2.2.0  
...
| `0xC6` | TURN | `schema:u8` `len:u16be` `body` (ADUHEAD schema `0x01`; see appendix Delay Hull) |
| `0xC7-0xCE` | Reserved | Skip-by-length when header present |
...
**Fence escaping inside args.** Args MUST NOT contain raw `0x9A` or `0x9B`. Escape (3 bytes, args only): `0xC1 0x1B <byte>`. Thus `C1 1B 9A` means literal `0x9A`. Bare fence bytes in args MUST hard-fault (nested frames are forbidden in v1). Truncated `C1 1B` (missing the literal byte) MUST hard-fault; it is not a pair of literals. `len` counts **wire** (escaped) bytes.
...
- if `L` in `0xF0-0xF4`: read 3 conts; `v = ((L & 0xF) << 15) | ((c0 & 0x1F) << 10) | ((c1 & 0x1F) << 5) | (c2 & 0x1F)`; `cp = v + 0x4280`; reject if `cp` is a UTF-16 surrogate (`U+D800..U+DFFF`)
...
- Surrogates as scalars, as 4-byte residual (`U+D800..U+DFFF` in the F0-F4 map; example `F1 A5 AC A0` = U+D800), or as LONG payloads
...
## Appendix: Delay Hull / TURN (append-only, 2.2.0)

This appendix does **not** change P0-P2 parse law, Cont freeze (`0xA0-0xBF`, 5-bit), agent opcodes, or ASCENT-D P9 defaults. It assigns reserved P2 lead `0xC6`. Decoders older than ascent-wire 2.2.0 hard-fault on an unknown `0xC6` lead. 2.2.0 skips an unknown TURN schema by length.

**Non-claims:** A TURN unit is one request document or one response document for a contact window. It is not interactive Grok. It is not a Bundle Protocol Agent and not a convergence layer. Starlink IP stays ASCENT-E-LEO. Full P9 is the delay spool only, and it is not stacked on PATHHINT CRC. One symbol error inside RS(255,223) is corrected. Erase happens when the outer decode fails.

### H.1 TURN lead

```
0xC6 <schema:u8> <len:u16be> <body:len>
```

- `len` cap for skip: 16384. Schema `0x01` body cap: 256.
- Unknown `schema` with readable `len`: **skip-by-length**; `applied=false`.
- Truncation before `len` is readable: hard-fault.

### H.2 ADUHEAD v1 (`schema = 0x01`)

Body 27 bytes, plus 4 if `FLAG_CRC` (bit0). v1 goldens set CRC.

| Offset | Field | Notes |
|--------|-------|-------|
| 0 | `flags:u8` | bit0 CRC; bit1-7 MBZ |
| 1 | `session:u64be` | session id |
| 9 | `turn:u32be` | turn inside the session |
| 13 | `corr:u64be` | pairs a request with its response |
| 21 | `lifetime_s:u32be` | usefulness window, seconds after enqueue |
| 25 | `role:u8` | 1 request, 2 response, 3 ack-erase |
| 26 | `reserved:u8` | MUST be 0 |
| 27 | `crc32:u32be` | if FLAG_CRC; IEEE CRC-32 of bytes 0..26 |

**Fail-closed:** CRC fail, reserved flags, bad length, reserved byte not 0, role outside 1..3, unknown schema: **erase** (`applied=false`). A duplicate `session+turn+corr` at the contact consumer is a no-op, not a second TOOL. Lifetime expiry drops the document. Missing CHUNK `END` discards the reassembly buffer (SPEC C.3.2).

Goldens: `tests/test_delay_hull.py`, `tests/run_js_lock.js`. Narrative: `docs/DELAY-HULL.md`.
