...
(function () {
  ...
  var PLANES = [
    ...
    },
    {
      ...
      detail:
        "Fenced agent control: STOP, ROLE, TOOL, THINK, HANDOFF, CAP, SAFETY. Bare 0x9A/0x9B inside args hard-faults. Truncated C1 1B escape hard-faults. THINK stays opaque on shared logs. SAFETY marks untrusted tool results as data.",
      example: "fencefault",
    },
    {
      id: "P6",
  ...
  var GALLERY = [
    ...
    { id: "surrguard", label: "4-byte surrogate", build: buildSurrogateGuard },
    { id: "fencefault", label: "Agent fence fault", build: buildFenceFault },
  ...
  var SPEC_SECTIONS = [
    ...
    },
    {
      ...
      label: "Agent frames",
      html:
        "<h3>P5 Agent frame</h3><p>Fenced, length-declared control for LLM tool loops. Bare <code>0x9A</code>/<code>0x9B</code> inside args MUST hard-fault. Truncated <code>C1 1B</code> escape MUST hard-fault. CRITICAL unknown opcodes hard-fault.</p><pre>0x9A AGENT_OPEN\n0xC1 ver opcode:u16be flags len:u16be args\n0x9B AGENT_CLOSE\n\nSTOP=1 ROLE=2 TOOL=3 THINK=4\nHANDOFF=5 CAP=6 SAFETY=7\n9A C1 01 00 04 00 00 01 9A 9B  -&gt; reject bare fence</pre>",
      sample: "fencefault",
    },
    {
      id: "mm",
    ...
    },
    {
      ...
      html:
        "<h3>Encoding guard (LONG overlongs + 4-byte surrogates)</h3><p>LONG <code>F5 03</code> + u24be MUST reject ASCII overlongs and non-minimal width. 4-byte residual MUST hard-fault UTF-16 surrogates. Paste <code>F5 03 00 00 41</code> (overlong A) or <code>F1 A5 AC A0</code> (U+D800).</p><pre>F5 03 00 00 41  -&gt; reject overlong ASCII\nF1 A5 AC A0     -&gt; reject surrogate U+D800\nF5 03 02 C2 80  -&gt; U+2C280 first legal LONG</pre>",
      sample: "encguard",
    },
    {
      id: "d",
  ...
  function buildSurrogateGuard() {
    // Illegal 4-byte residual U+D800 (F1 A5 AC A0). Decoders MUST hard-fault.
    return C.fromHex("F1A5ACA0");
  }

  function buildFenceFault() {
    // Bare OPEN 9A inside declared THINK args. Nested fence MUST hard-fault.
    return C.fromHex("9AC10100040000019A9B");
  }
  ...
  function wireDelayHull() {
    ...
    function show(unit, note) {
      var evs = C.decodeStream(unit);
      var turn = null;
      for (var i = 0; i < evs.length; i++) {
        if (evs[i].kind === "turn") turn = evs[i];
      }
      var pre = document.getElementById("delay-fields");
      var st = document.getElementById("delay-status");
      if (pre) {
        pre.textContent =
          "SIM. Not interactive Grok. Not a faster radio.\n" +
          (note || "") +
          "\n\n" +
          (turn ? JSON.stringify(turn, null, 2) : "no TURN") +
          "\n\nhex " +
          C.hexOf(unit).toUpperCase();
      }
      if (st) {
        st.textContent = "SIM. Not interactive Grok. Not a faster radio. " + (note || "");
        st.className = "status-line " + (turn && turn.applied ? "ok" : "warn");
      }
    }
    function encodeHeld() {
      var opts = {
        session: num("delay-session"),
        turn: num("delay-turn"),
        corr: num("delay-corr"),
        lifetimeS: num("delay-life"),
        role: num("delay-role") || 1,
        crc: true,
      };
      var unit = C.encodeTurn(opts);
      held = {
        unit: unit,
        enqueuedAt: num("delay-now"),
        key: opts.session + ":" + opts.turn + ":" + opts.corr,
      };
      return unit;
    }
    function step() {
      if (!held) {
        var st0 = document.getElementById("delay-status");
        if (st0) {
          st0.textContent = "Encode a sample first. SIM. Not interactive Grok.";
          st0.className = "status-line warn";
        }
        return;
      }
      var evs = C.decodeStream(held.unit);
      var turn = evs[0];
      var now = num("delay-now");
      var open = num("delay-open");
      var close = num("delay-close");
      var note;
      if (!turn || !turn.applied) {
        note = "SKIPPED. " + ((turn && turn.reason) || "no turn");
      } else if (now < open || now >= close) {
        note = "HELD. Contact window is closed.";
      } else if (now > held.enqueuedAt + turn.lifetime_s) {
        note = "DROPPED. Lifetime expired. No TOOL.";
      } else if (seen[held.key]) {
        note = "NO-OP. Duplicate correlation. No second TOOL.";
      } else {
        seen[held.key] = true;
        note = "DELIVERED. One document inside the window.";
      }
      show(held.unit, note);
    }
    var enc = document.getElementById("btn-delay-encode");
    if (enc) {
      enc.addEventListener("click", function () {
        try {
          seen = {};
          var unit = encodeHeld();
          setStream(unit, { forceText: true, flash: true });
          show(unit, "Encoded. Step the clock to hold, deliver, or drop.");
        } catch (e) {
          var st = document.getElementById("delay-status");
          if (st) {
    ...
    var stepBtn = document.getElementById("btn-delay-step");
    if (stepBtn) stepBtn.addEventListener("click", step);
    var dup = document.getElementById("btn-delay-dup");
    if (dup) {
    ...
    var ins = document.getElementById("btn-delay-insert");
    if (ins) {
      ins.addEventListener("click", function () {
        try {
          var unit = encodeHeld();
          insertBytes(unit, "append");
          show(unit, "Inserted. ASCII around it stays ASCII.");
        } catch (e2) {
          alert("TURN error: " + e2.message);
        }
      });
    }
  }
  ...
  function buildCmdkCatalog() {
    ...
    [
      ...
      ["Navigate", "Delay Hull", "#delay-hull", ""],
