  ...
  function wireExport() {
    ...
    if (jsonBtn) {
      jsonBtn.addEventListener("click", async function () {
        ...
        jsonBtn.textContent = "JSON copied";
        setTimeout(function () {
          jsonBtn.textContent = "JSON units";
        }, 1000);
      });
    }
  }
  ...
  function wireMeta() {
    ...
    if (t) {
      t.addEventListener("click", function () {
        var bytes = C.encodeText("ASCENT Nexus - Wire Lab 2.0\n", {
          header: true,
          nonAscii: "reject",
        });
        setStream(bytes, { forceText: true, flash: true });
