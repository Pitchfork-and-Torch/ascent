...
A second decode hole: the 4-byte residual (`U+4280..U+2C27F`) includes UTF-16 surrogates `U+D800..U+DFFF`. Encoders already reject them; decoders MUST hard-fault a 4-byte unit in that range (example `F1 A5 AC A0` = U+D800). No best-effort lone surrogate in text.
...
| `F1 A5 AC A0` | reject UTF-16 surrogate U+D800 (4-byte residual) |
| `F1 A7 AB BF` | reject UTF-16 surrogate U+DFFF |

## Wire Lab

Nexus gallery sample **Encoding guard** loads `F503000041`. Decode status must show a hard fault, not the letter A.

**4-byte surrogate** loads `F1A5ACA0` (U+D800). Decode status must hard-fault, not emit a lone surrogate.

**Agent fence fault** loads `9AC10100040000019A9B` (bare OPEN inside THINK args). Decode status must hard-fault nested fence, not skip it.
